...
class Decoder(srd.Decoder):
    ...
    options = (
        {'id': 'arbbitrate', 'desc': 'ArbBitrate (bits/s)', 'default': 500000},
        {'id': 'databitrate', 'desc': 'DataBitrate (bits/s)', 'default': 5000000},
        {'id': 'arbsample_point', 'desc': 'Arb Sample point (%)', 'default': 70.0},
        {'id': 'datasample_point', 'desc': 'Data Sample point (%)', 'default': 70.0},
    )
    annotations = (
    ...
    def setSamplerate(self, flag):
        self.bit_width = float(self.samplerate) / float(self.options['arbbitrate'])
        self.sample_point = (self.bit_width / 100.0) * self.options['arbsample_point']

        self.bit_width1 = float(self.samplerate) / float(self.options['arbbitrate'])
        self.sample_point1 = (self.bit_width / 100.0) * self.options['arbsample_point']
        self.bit_width2 = float(self.samplerate) / float(self.options['databitrate'])
        self.sample_point2 = (self.bit_width / 100.0) * self.options['datasample_point']

    def metadata(self, key, value):
        if key == srd.SRD_CONF_SAMPLERATE:
            self.samplerate = value
            self.setSamplerate(0)
            #self.bit_width = float(self.samplerate) / float(self.options['arbbitrate'])
            #self.sample_point = (self.bit_width / 100.0) * self.options['arbsample_point']
    ...
    def dom_edge_seen(self, force = False):
        if self.canfd_flag == 1:
            self.dom_edge_snum = self.samplenum
            self.dom_edge_bcount = self.curbit
        
            #self.dom_edge_snum = self.samplenum + int(self.bit_width2 * 650)
            #self.curbit += int(self.bit_width2 * 650)
            #self.dom_edge_bcount = self.curbit
            #samplenum += int(self.bit_width2 * 650)
        else:
            self.dom_edge_snum = self.samplenum
            self.dom_edge_bcount = self.curbit
    ...
    def get_sample_point_data(self, bitnum):
        samplenum = self.dom_edge_snum
        samplenum += int(self.bit_width2 * (bitnum - self.dom_edge_bcount))
        samplenum += int(self.sample_point)
        return samplenum
        
    def get_sample_point_end(self, bitnum):
        samplenum = self.dom_edge_snum
        samplenum += int(self.bit_width * (bitnum - self.dom_edge_bcount))
        samplenum += int(self.sample_point)
        return samplenum
    ...
    def decode_standard_frame_classics(self, can_rx, bitnum):
        ...
        return False
    def decode_standard_frame_fd(self, can_rx, bitnum):
        # Bit 14: RB0 (reserved bit)
        # Has to be sent dominant, but receivers should accept recessive too.
        if bitnum == 14:
            #return self.decode_frame_end(can_rx, bitnum)
            ...
            self.putx([7, ['Reserved bit 0: %d' % can_rx,
                           'RB0: %d' % can_rx, 'RB0']])
            #...
            self.bit_width = self.bit_width2
            self.sample_point = self.sample_point2
        # Remember start of DLC (see below).
        elif bitnum == 17:
            self.ss_block = self.samplenum
        ...
        # Bits 15-18: Data length code (DLC), in number of bytes (0-8).
            ...
            self.dlc = int(''.join(str(d) for d in self.bits[17:20 + 1]), 2)
            if self.dlc == 9:
                self.dlc = 12
            elif self.dlc == 10:
                self.dlc = 16
            ...
            self.putb([10, ['Data length code: %d' % self.dlc,
                            'DLC: %d' % self.dlc, 'DLC']])
            self.last_databit = 20 + (self.dlc * 8)
            if self.dlc > 8:
                self.putb([16, ['Data length code (DLC) > 8 is not allowed']])

        # Remember all databyte bits, except the very last one.
        elif bitnum in range(21, self.last_databit):
            self.ss_databytebits.append(self.samplenum)
    ...
    def decode_standard_frame(self, can_rx, bitnum):
        ...
        if bitnum == 14:
            if can_rx == 1:
                ...
                self.canfd_flag = 0
        if self.canfd_flag == 1:
            ret = self.decode_standard_frame_fd(can_rx, bitnum)
        else:
            ret = self.decode_standard_frame_classics(can_rx, bitnum)
    ...
    def decode(self):
        ...
        while True:
            ...
            if self.state == 'IDLE':
                ...
                self.bit_width = self.bit_width1
                self.sample_point = self.sample_point1
                
                #if self.canfd_flag == 1:
                #    canfd_num = 500
                #    while canfd_num > 0:
                        # Wait for a dominant state (logic 0) on the bus.
                        #(can_rx,) = self.wait([{'skip': 2000}, {0: 'r'}])
                #        (can_rx,) = self.wait({0: 'e'})
                #        canfd_num -= 1
                #else:
                    # Wait for a dominant state (logic 0) on the bus.
                ...
                self.state = 'GET ARB BITS'
            elif self.state == 'GET ARB BITS':
                # Wait until we're in the correct bit/sampling position.
                pos = self.get_sample_point(self.curbit)
                (can_rx,) = self.wait([{'skip': pos - self.samplenum}, {0: 'e'}])
